...
--[[
    GECICI DEBUG (2026-08-26, ROUND 4 — previewPed'in strict routing bucket icinde
    77-86ms'de silinme arastirmasi icin eklendi, root cause kesinlesince KALDIRILACAK).
    Client (preview_manager.lua) previewPed'in network ID'sini gonderir; biz SUNUCU
    tarafinda SABIT zaman noktalarinda (0/10/20/50/60/70/80/90/100/150/200/500ms)
    su anlik goruntuyu (snapshot) aliriz:
        - netId -> entity cozulebiliyor mu (NetworkGetEntityFromNetworkId)
        - entity'nin routing bucket'i (GetEntityRoutingBucket)
        - oyuncunun routing bucket'i (GetPlayerRoutingBucket)
        - ikisi eslesiyor mu (bucketsMatch)
        - entity owner (NetworkGetEntityOwner) + model (GetEntityModel)
    Amac: silinmenin routing bucket uyumsuzlugu ile zamansal/mantiksal olarak
    orantili olup olmadigini SABIT checkpoint'lerle kanitlamak (onceki versiyon
    sadece "durum degisince" yazdiriyordu, 77-86ms'lik pencereyi hangi checkpoint'te
    gectigini net gostermiyordu). Production davranisini DEGISTIRMEZ (sadece print +
    kisa sureli, kendi kendine biten bir izleme thread'i). ROUTING BUCKET DEGISTIRME
    YOK — sadece OKUMA/gozlem.
]]
RegisterNetEvent('bitirim:debugPreviewNetInfo', function(netId, clientCreatedAt)
    local src = source

    local function snapshot(label, t0)
        local ent = NetworkGetEntityFromNetworkId(netId)
        local exists = ent ~= 0 and DoesEntityExist(ent)
        local okP, pBucket = pcall(GetPlayerRoutingBucket, src)
        local okE, eBucket, okO, owner, okM, model = false, nil, false, nil, false, nil
        if exists then
            okE, eBucket = pcall(GetEntityRoutingBucket, ent)
            okO, owner = pcall(NetworkGetEntityOwner, ent)
            okM, model = pcall(GetEntityModel, ent)
        end
        local pB = okP and pBucket or nil
        local eB = okE and eBucket or nil
        local match = (pB ~= nil and eB ~= nil) and (pB == eB) or nil
        print(('^6[bitirim-debug-srv] %-8s netId=%s src=%d handle=%s exists=%s entityBucket=%s playerBucket=%s bucketsMatch=%s owner=%s model=%s^7')
            :format(label, tostring(netId), src, tostring(ent), tostring(exists),
                tostring(eB), tostring(pB), tostring(match), tostring(okO and owner), tostring(okM and model)))
        return exists
    end

    CreateThread(function()
        local t0 = GetGameTimer()
        -- 77-86ms'lik olum penceresini siki bracket'lemek icin ekstra checkpoint'ler
        -- (60/70/80/90) eklendi; digerleri sizin istediginiz sabit noktalar.
        local checkpoints = { 0, 10, 20, 50, 60, 70, 80, 90, 100, 150, 200, 500 }
        local lastExists = nil
        for i = 1, #checkpoints do
            local target = checkpoints[i]
            local waitFor = target - (GetGameTimer() - t0)
            if waitFor > 0 then Wait(waitFor) end
            local nowExists = snapshot(('t+%dms'):format(GetGameTimer() - t0), t0)
            if lastExists == true and nowExists == false then
                print(('^1[bitirim-debug-srv] ILK KEZ KAYBOLDU (server gozunden): netId=%s ~t=%dms^7')
                    :format(tostring(netId), GetGameTimer() - t0))
            end
